...
//#define GxGDEH029A1_PU_DELAY          10

#define DELAY_UPDATE                  vTaskDelay(300)   // Tiempo que espera para actualizar la pantalla
#define DELAY_BUSY                    vTaskDelay(2)     // Tiempo que espera mienstra esta ocupado el display
...
GxGDEH029A1::GxGDEH029A1(GxIO& io, int8_t rst, int8_t busy) :
  GxEPD(GxGDEH029A1_WIDTH, GxGDEH029A1_HEIGHT), IO(io), _current_page(-1), _using_partial_mode(false), _diag_enabled(false), _rst(rst), _busy(busy)
{
}
...
void GxGDEH029A1::updateWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h, bool using_rotation){
  ...
  _Update_Part();     // Esto le toma 411 mseg
  DELAY_UPDATE;
...
void GxGDEH029A1::_waitWhileBusy(const char* comment){
  ...
  while (1)
  {
    ...
    {
      if (_diag_enabled) Serial.println("ERROR: Demasiado tiempo de espera en el display!!");
      break;
    }
  }
